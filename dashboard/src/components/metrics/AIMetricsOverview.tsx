...
export function AIMetricsOverview({ data }: AIMetricsOverviewProps) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-gray-900">AI-Assisted Development</h2>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <MetricCard
          title="AI Commits"
          value={data.summary.totalAICommits.toString()}
          subtitle={`${formatPercent(data.summary.aiRatio)} of all commits`}
          icon={
        ...
        <MetricCard
          ...
          value={`${highAdoptionUsers} / ${totalUsers}`}
      ...
      {data.byTool.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-sm font-medium text-gray-700 mb-4">AI Tools Usage</h3>
          <div className="space-y-3">
            {data.byTool.map((tool) => (
              <div key={tool.tool} className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-500">{tool.commits} commits</span>
      ...
      {data.byUser.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-sm font-medium text-gray-700 mb-4">User AI Adoption</h3>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="p-3 bg-gray-50 rounded-lg">
              <p className="text-2xl font-semibold text-gray-900">
                {data.byUser.filter((u) => u.ratio === 0).length}
              </p>
              <p className="text-xs text-gray-500 mt-1">No AI usage</p>
            </div>
            <div className="p-3 bg-blue-50 rounded-lg">
              <p className="text-2xl font-semibold text-blue-700">
                {data.byUser.filter((u) => u.ratio > 0 && u.ratio < 0.3).length}
              </p>
              <p className="text-xs text-gray-500 mt-1">1-30% AI commits</p>
            </div>
            <div className="p-3 bg-green-50 rounded-lg">
              <p className="text-2xl font-semibold text-green-700">
                {data.byUser.filter((u) => u.ratio >= 0.3).length}
              </p>
              <p className="text-xs text-gray-500 mt-1">&gt;30% AI commits</p>
            </div>
          </div>
        </div>
      )}
